            ...
            if noteNum ~= nil then
                ...
                elseif self.enable_note_highlighting then
                    ...
                    elseif noteName == music.NOTE_NAMES[self.root_note] then
                      ...
                      if grid_utils.nb_levels(grid) == 1 then
                        i = 15
                      end
                      grid.og_led(grid, x + xOff, y + yOff, i)
                    elseif self:is_note_in_scale(noteNum) then
                        if self.layout_mode == 1 then -- Chromatic mode
                          grid.og_led(grid, x + xOff, y + yOff, 3 + selectedOff + animOff)
                        elseif self.layout_mode == 2 then
                          grid.og_led(grid, x + xOff, y + yOff, selectedOff + animOff)
                        elseif self.layout_mode == 3 then
                          grid.og_led(grid, x + xOff, y + yOff, 3 + selectedOff + animOff)
                        end
